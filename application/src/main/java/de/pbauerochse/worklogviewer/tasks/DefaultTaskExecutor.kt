...
object DefaultTaskExecutor : TaskExecutor {
    ...
    private val LOGGER = LoggerFactory.getLogger(DefaultTaskExecutor::class.java)
    private val EXECUTOR = Executors.newFixedThreadPool(5)

    override fun <T> startBackgroundTask(task: WorklogViewerTask<T>): Future<T> {
        return prepareTaskForExecution(task, runningBackgroundTasksList)
    }

    override fun <T> startTask(task: WorklogViewerTask<T>): Future<T> {
        return prepareTaskForExecution(task, runningForegroundTasksList)
    }
